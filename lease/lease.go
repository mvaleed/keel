...
// Keepalive renews the lease every ttl/3 until stop runs or ctx ends.
// Use it to hold a lease across a call that is longer than ttl.
//
// The returned stop function waits for the goroutine and reports the
// error that ended it. A nil error means stop or ctx ended it, and
// ErrLeaseLost means another owner took the resource over. The caller
// must stop its work when the lease is lost, because a later write fails
// and the other holder is already doing the same work.
func Keepalive(ctx context.Context, lk Locker, l *Lease, ttl time.Duration) (stop func() error) {
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				...
				return
			case <-ticker.C:
				if err := lk.Renew(ctx, l, ttl); err != nil {
					result <- err
					return
				}
			}
